 ...
 *
 * Exports:
 *   upsertSpace(crawledData)       → { action, spaceId, newReviews, changedFields }
 ...
 *   3. placeId
 *
 * On INSERT  — creates space + inserts reviews into separate collection.
...
async function upsertSpaceSource({ spaceId, opgId, crawledData, now, status = 'completed', error = null }) {
  try {
    ...
    const filter = providerPlaceId
      ...
      : (providerUrlHash ? { provider, providerUrlHash } : { spaceId: spaceId, provider });
...
async function upsertPhotos(spaceId, photoUrls = [], now, opgId) {
...
async function upsertCrawlMeta(spaceId, rawMeta, now, opgId) {
...
function applyDerivedSignals(normalizedData, crawledData) {
  ...
  const sentiment = analyzeSpaceSentiment(crawledData.reviews);
...
async function findExistingSpace(crawledData) {
  ...
  if (placeId) {
    ...
    if (sourceDoc?.spaceId) {
      const boundSpace = await Space.findById(sourceDoc.spaceId).lean();
      if (boundSpace) return boundSpace;
    }
  }
  ...
  // Tier 5: Phone number match (for rebranded spaces at different addresses)
  if (phone) {
...
async function insertReviews(spaceId, rawReviews = [], opgId) {
  if (!rawReviews.length) return 0;

  const docs = buildReviewDocs(spaceId, rawReviews);
...
async function mergeReviews(spaceId, rawReviews = [], opgId) {
  if (!rawReviews.length) return 0;
...
/**
 * @param {Object} crawledData  — the structured space object from spaceProcessor
 * @returns {{ action: string, spaceId: ObjectId, newReviews: number, newPhotos: number, changedFields: string[] }}
 */
async function upsertSpace(crawledData) {
  ...
  try {
    ...
    if (!existing) {
      ...
      // Generate a unique public OPG ID before creating the space
      const opgId = await generateUniqueOpgId(Space);
      normalizedData.opgId = opgId;

      // 1. Create Space (Raw array values mapped correctly to raw field)
      ...
      logger.info(`[INSERT] "${crawledData.name}" → new space added (opgId: ${opgId})`);
    ...
    if (diffs.length) {
      await writeChangeLogs(spaceId, diffs, now);
    ...
    const [reviewResult, photoResult] = await Promise.all([
      mergeReviews(spaceId, crawledData.reviews, opgId),
      upsertPhotos(spaceId, crawledData.photoUrls, now, opgId),
      upsertCrawlMeta(spaceId, crawledData.crawlMeta, now, opgId),
      upsertSpaceSource({ spaceId, opgId, crawledData: normalizedData, now, status: 'completed' }),
    ]);
    ...
    if (somethingChanged) {
      ...
      // Still update crawlMeta timestamp so we know this space was visited
...
/**
 * @param {Array} spacesArray  — array of structured space objects
